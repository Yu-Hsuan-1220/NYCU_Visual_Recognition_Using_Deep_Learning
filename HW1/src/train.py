...
def parse_args():
    ...
    parser.add_argument('--train_dir', type=str,
                        default='../dataset/data/train',
                        help='Path to training data directory')
    parser.add_argument('--val_dir', type=str,
                        default='../dataset/data/val',
    ...
    parser.add_argument('--label_smoothing', type=float, default=0.1,
                        help='Label smoothing factor (if smoothing is set)')
...
def train_one_epoch(
    ...
    for batch_idx, (images, labels) in enumerate(pbar):
        ...
        # Map soft targets back to hard class indices
        if labels.ndim == 2:
...
def main():
    ...
    if not args.no_wandb:
        def_run_name = f"{args.backbone}_bs{args.batch_size}_lr{args.lr}"
        run_name = args.wandb_run_name or def_run_name
        wandb.init(
    ...
    for epoch in range(start_epoch, args.epochs):
        if epoch < args.warmup_epochs:
            if epoch == 0:
                print(f"\nWarmup: Freezing backbone for {args.warmup_epochs}")
        ...
        c_lr_fc = optimizer.param_groups[1]['lr'] if \
            len(optimizer.param_groups) > 1 else c_lr

        print(f"\nEpoch {epoch} Summary:")
        print(f"  Train Loss: {train_loss:.4f}, Train Acc: {train_acc:.2f}%")
        print(f"  Val Loss: {val_loss:.4f}, Val Acc: {val_acc:.2f}%")
        print(f"  LR (backbone): {c_lr:.6f}, LR (FC): {c_lr_fc:.6f}")
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            best_p = os.path.join(args.save_dir, f'best_{args.backbone}.pth')
            save_checkpoint(
                model, optimizer, scheduler, epoch, best_acc, best_p
            )
            print(f"New best accuracy: {best_acc:.2f}%")

        if (epoch + 1) % args.save_freq == 0:
            ckpt_p = os.path.join(
                args.save_dir, f'{args.backbone}_epoch{epoch}.pth'
            )
            save_checkpoint(
                model, optimizer, scheduler, epoch, best_acc, ckpt_p
            )

    final_p = os.path.join(args.save_dir, f'final_{args.backbone}.pth')
    save_checkpoint(
        model, optimizer, scheduler, args.epochs - 1, best_acc, final_p
    )
