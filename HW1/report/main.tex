...
The dataset consist of approximately 21,024 traing/validation images and 2344 test images. The primary challenge lies 
...
To maximize test accuracy, we implemented a 7-view Test-Time Augmentation (TTA) strategy. For each test image, we generated seven versions: the original center-cropped image, four corner crops, horizontal flips and rotations. The final prediction was determined by Soft Voting, which averages the softmax probability distributions across all five views to produce a more reliable class assignment.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textbf{Augmentation}: RandAugment, Mixup ($\alpha=1.0$), and CutMix ($\alpha=1.0$).
        \item \textbf{Regularization}: Label Smoothing (0.1) and Dropout (0.5).
    ...
    \begin{itemize}
        ...
        \item \textbf{Scheduler}: Cosine Annealing with a Warmup phase (5 or 10 epochs).
    \end{itemize}
    ...
    \end{itemize}
    \item \textbf{Inference Strategy}: 7-view TTA (including center/corner crops, horizontal flips, and rotations) combined with Soft Voting before the final Majority Vote ensemble.
\end{itemize}
...
We conducted a experiment show in Figure \ref{fig:without_aug} and found that without the diverse pixel-level and geometric transformations 
...
The experiment in Figure \ref{fig:without_mix} showed that without Mixup and CutMix, the validation accuracy would drop, and the model would be more prone to overfit the training data.
...
The experiment in Figure \ref{fig:without_wd} is the expriment without weight decay. we compared the training dynamics of ResNet-152 with and without Weight Decay (WD) ($\lambda = 5 \times 10^{-4}$).
